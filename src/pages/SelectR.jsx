...
import "./Select.css";
import { patchSelectCity } from "../api/departure";
...
const CitySelector =()=>{
  ...
  useEffect(()=>{
    const updateCity = async()=>{
      try{
        const selectedRegion = "가져와야함";
        const selectedId = 123; 

        const response = await patchSelectCity(selectedRegion,selectedId);
        console.log("api response:", response);
        setRegion(selectedRegion);
        setId(response.data.id);

      } catch(error){
        console.error("city notupdate : ", error);
      }
    };
    updateCity();
  }, [token]);
